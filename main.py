...
def emailInfo(id,sessauth,sessid,emailName):
    ...
    r = requests.post('http://pimar.cn/?_task=mail&_uid='+id+'&_mbox=INBOX&_framed=1&_action=preview', headers=header)
...
def newUser(emailname,password,emailPassword):
    ...
    if 'idToken' in r.json().keys() :
            ...
            lookup = requests.post('https://identitytoolkit.googleapis.com/v1/accounts:lookup?key={}'.format(apikey),data=lookupDetails)
            print(lookup.json())
...
if __name__ == '__main__':
    ...
    newUser("ljzteh@klonsa.cn",PASSWORD,"988847")